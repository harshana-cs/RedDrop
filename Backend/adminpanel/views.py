...
def test_sms_view(request):
    from adminpanel.sms import send_sms, _normalize_phone
    from django.conf import settings
    import json

    # ── Parse body safely (handles both DRF parsed and raw string) ──
    data = request.data
    ...
    message = data.get("message", "RedDrop test SMS")

    normalized  = _normalize_phone(phone)
    sms_token   = getattr(settings, "SMS_TOKEN", None)
    sms_from    = getattr(settings, "SMS_FROM",  None)
    ...
    return Response({
        ...
        "sms_token_set": bool(sms_token and str(sms_token).strip()),
        "sms_from":      sms_from,
        "sent":          result,
        "debug": {
            "token_length": len(str(sms_token).strip()) if sms_token else 0,
            "valid_format": bool(normalized and len(normalized) == 10 and normalized.startswith("9")),
        }
    })